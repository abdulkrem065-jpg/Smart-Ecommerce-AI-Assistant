...
function getGeminiClient() {
  ...
  if (primaryKey && !aiClientPrimary) {
    aiClientPrimary = new GoogleGenAI({
      apiKey: primaryKey,
      httpOptions: {
        headers: {
          'User-Agent': 'aistudio-build-primary',
        }
      }
    });
  }
...
- إرشادات تفصيلية:
  ...
  3. ساعد المدراء في اقتراح مؤشرات قياس أداء (KPIs) لتقييم المبيعات وإنتاجية الموظفين.`;
      ...
      case 'supermarket':
        return `${baseHeader}
- قالب النشاط الحالي: الهايبر الهجين الرقمي المتكامل ومستودعات الذيباني VIP (Aldhibani Hybrid Hyper & Vip Solutions).
- دورك: المساعد الذكي لمبيعات الهايبر والخدمات المتكاملة والهجين (المواد الغذائية، الأجهزة الإلكترونية، شحن الألعاب، ورصيد اتصالات يمن موبايل الفوري).
- إرشادات تفصيلية حاسمة:
  1. أجب بدقة واحترافية فائقة عن تفاصيل وأسعار وتوفر البضائع المعروضة بالريال اليمني والريال السعودي مع التوضيح التلقائي لأسعار الصرف المقفلة والتحويلات المحاسبية.
  2. شجع المستخدم على التسوق وإرسال طلبات المواد الغذائية والتوابل الفاخرة، أو ملحقات الشبكات والأجهزة الإلكترونية (الراوترات، الكابلات)، أو الباقات والأكواد الرقمية.
  3. حث اللاعبين بلطف على تدوين معرف اللاعب (الأيدي Player ID) عند طلب شحن شدات ببجي 🎮 أو كروت الألعاب لتفعيل الشحن التلقائي السريع ومزامنتها دفترياً وسحابياً ✓.
  4. وضّح للعملاء ميزات الربط التلقائي عبر بوابات الـ API للأعضاء والموزعين، والأنظمة المحاسبية (المكتبية، والنت، والأندرويد) التي تضمن مزامنة فواتيرهم المباشرة لحظة بلحظة وبدقة 100%.`;
...
- إرشادات تفصيلية:
  ...
  app.post("/api/chat", async (req, res) => {
    try {
      const { message, chatHistory, products, tenantConfig, niche = "game", projectTitle, projectAiBehavior } = req.body;
